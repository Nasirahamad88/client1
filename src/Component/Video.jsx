...
const responsive = {
  ...
  desktop: {
    ...
    items: 6, // Adjusted to 4 items for better desktop layout
  },
  laptop: {
    breakpoint: { max: 2000, min: 924 },
    items: 5, // Adjusted to 4 items for better laptop layout
  },
  tablet: {
    breakpoint: { max: 1024, min: 464 },
    items: 3, // 3 items on tablet for better fit
  },
  mobile: {
    breakpoint: { max: 464, min: 0 },
    items: 3, // 2 items on mobile for better fit
  },
};
...
const VideoCarousel = () => {
  ...
  const CustomLeftArrow = ({ onClick }) => (
    <button onClick={onClick} className="absolute left-0 z-10 rounded-full flex items-center p-2 bg-custom-purple">
      <FontAwesomeIcon icon={faArrowLeft} className="text-white lg:text-3xl hover:text-black" />
    </button>
  );

  const CustomRightArrow = ({ onClick }) => (
    <button onClick={onClick} className="absolute right-0 z-10 rounded-full flex items-center p-2 bg-custom-purple">
      <FontAwesomeIcon icon={faArrowRight} className="text-white lg:text-3xl hover:text-black" />
    </button>
  );

  return (
    <div className="video-carousel flex flex-col items-center w-[95%] lg:w-[75%] mx-auto py-20 relative">
        ...
        {videos.map((video, index) => (
          <div key={index} className="flex flex-col items-center">
            {showVideoIndex === index ? (
              <video
                className="lg:w-44 w-24  h-40  lg:h-96 md:h-96 rounded-2xl" // Adjusted dimensions for mobile
                controls
                autoPlay
                muted
              ...
              <div
                className="relative bg-slate-800 lg:w-44 w-24  h-40  lg:h-96 md:h-96 rounded-2xl cursor-pointer flex items-center justify-center"
